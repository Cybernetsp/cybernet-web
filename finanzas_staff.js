...
window.cargarDistribuidores = function () {
    ...
    <tr>
      <td colspan="2" style="text-align: center; padding: 40px; color: var(--ios-blue);">
        <div style="display: flex; flex-direction: column; align-items: center; gap: 12px;">
          ...
          <span style="font-weight: 600;">Sincronizando distribuidores...</span>
        </div>
      </td>
    </tr>`;
  ...
  fetch("https://api.cybernetsp.com/obtener_distribuidores.php")
    ...
    .then((res) => {
      if (res.status === "success") {
        if (res.data.length === 0) {
          tbody.innerHTML = `<tr><td colspan="2" style="text-align: center; padding: 30px; color: var(--text-secondary);">No hay distribuidores registrados.</td></tr>`;
          return;
        }
            ...
            <tr class="distri-row-item" style="background: ${bgRow}; border-bottom: 1px solid rgba(255,255,255,0.03);">
              ...
              <td style="padding: 14px 18px; text-align: right;">
                <div style="display: flex; align-items: center; justify-content: flex-end; gap: 10px;">
                  <span style="font-size: 1.1rem; font-weight: 800; color: ${saldoNum > 0 ? "#30d158" : "#ff453a"}; font-family: monospace;">${saldoFormateado}</span>
                  <button type="button" onclick="window.copiarSaldoDistri(this, '${nombreLimpio.replace(/'/g, "\\'")}', '${saldoFormateado}')" style="background: rgba(255, 255, 255, 0.08); border: 1px solid rgba(255, 255, 255, 0.15); color: #ffffff; padding: 6px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 6px;">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg> Copiar
                  </button>
                </div>
              </td>
            </tr>`;
